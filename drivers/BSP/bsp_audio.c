...
extern uint16_t pInFFTBufferFHFull[];
...
void HAL_I2S_TxCpltCallback(I2S_HandleTypeDef *hi2s){
  ...
  if(!flgBufferCopyCompleteFull){
    // memcpy(pInFFTBufferFHFull, ((hi2s->pTxBuffPtr)+((uint32_t)(AUDIO_TOTAL_BUF_SIZE/4))), (AUDIO_TOTAL_BUF_SIZE/4));
    memcpy(pInFFTBufferFHFull,(hi2s->pTxBuffPtr), ((AUDIO_TOTAL_BUF_SIZE)/2));
    flgBufferCopyCompleteFull = true;
  }
}
...
void HAL_I2S_TxHalfCpltCallback(I2S_HandleTypeDef *hi2s){
  ...
  if(!flgBufferCopyComplete){
    memcpy(pInFFTBufferFH, (hi2s->pTxBuffPtr), ((AUDIO_TOTAL_BUF_SIZE)/2));
    flgBufferCopyComplete = true;
  }
}
